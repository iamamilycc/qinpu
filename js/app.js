...
(function () {
  ...
  function tutInit() {
    ...
    // 谱字术语速查
    var terms = [
      ['泛起 / 泛止', '泛音段开始 / 结束的标记：泛起省文作「⺈」下「巳」，泛止省文作「正」——谱中见「正」即泛音段到此为止'],
      ...
      ['再作', '从记号处再弹一遍'],
      ['从头再作', '整段从头重复'],
      ['曲终 / 操终', '全曲结束（常接泛音尾声）——操终省文作「𢫝」'],
      ['按音', '左手按弦而弹——有的谱以「宀」标示，本站减字以「左手指＋徽位」默示，不另标'],
      ['同声 / 应合', '左手带起、放合等与右手散音相应得声——省文：应合作「𢈈」'],
      ['锁 / 琐', '锁类总名省文作「巛」：背锁＝北下巛、短锁＝矢下巛、长锁六至十三声'],
      ['古写谱字', '齐撮（亠下最）＝撮的古写；对起、对过、对按＝掐起的古写；今谱罕用，读老谱时会遇到']
    ];
    if ($('tutTerms')) {
